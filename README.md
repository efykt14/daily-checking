...
2. Under **Security → Authentication → Get started**, enable the **Email/Password** sign-in provider.
3. Under **Databases & Storage → Firestore**, click **Create Database**, pick a region, and start in **production mode**.
4. Open the **Rules** tab and replace the contents with:
   ...
   ```
   This restricts each signed-in account to only ever read or write its own entries. Click **Publish**.
...
**How the sync works:** each entry is its own record in a live cloud database (Firestore), not a shared file — so saving from your phone and laptop within the same minute won't clobber each other the way a single shared file could. Every signed-in device also gets a **live update**: log something on your phone, and an already-open tab on your laptop updates within about a second, no refresh needed.
